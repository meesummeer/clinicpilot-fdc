...
// Report is payment-date based (NOT invoice date).
const {
  getSheetsClient,
  PATIENT_SHEET_ID,
  BILLING_SHEET_ID,
  getFirstSheetTitle,
  getOrCreateSheetByTitle,
  cors,
  ok,
  err
} = require('./_sheets');
...
async function clearBanding(sheets, spreadsheetId, sheetId) {
  ...
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId,
    resource: { requests: bands.map(b => ({ deleteBanding: { bandedRangeId: b.bandedRangeId } })) }
  });
}

function sortByPaymentDateThenCaseNo(a, b) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!/^\d{4}-\d{2}$/.test(String(ym || ''))) {
      return err('Invalid month format. Use YYYY-MM.', 400);
    }

    const sheets = getSheetsClient();

    // Join source 1: patient list (case no -> patient name)
    ...
    const patientRes = await sheets.spreadsheets.values.get({
      ...
      range: `${patientTitle}!A:B`
    });
    const patientNameByCaseNo = new Map();
    (patientRes.data.values || []).slice(1).forEach(r => {
      const caseNo = String(r[0] || '').trim();
      const name = String(r[1] || '').trim();
      if (caseNo) patientNameByCaseNo.set(caseNo, name);
    });

    // Join source 2: invoices (invoice_id -> procedure/lab_cost/cost/patient_id)
    const allInvoices = await getAllInvoices(sheets);
    const invoiceById = new Map(allInvoices.map(inv => [inv.id, inv]));

    // Base dataset: all payments filtered by payment date prefix YYYY-MM
    const allPayments = await getAllPayments(sheets);
    const monthPayments = allPayments.filter(p => String(p.date || '').startsWith(ym));
    ...
    const rows = monthPayments.reduce((acc, p) => {
      const inv = invoiceById.get(Number(p.invoice_id));
      if (!inv) return acc;

      const caseNo = String(inv.patient_id || p.patient_id || '').trim();
      if (!caseNo) return acc;

      acc.push([
        String(p.date || ''),
        caseNo,
        patientNameByCaseNo.get(caseNo) || '',
        inv.procedure || '',
    ...
    rows.sort(sortByPaymentDateThenCaseNo);
    ...
    // Preserve existing styling behavior.
    ...
    const requests = [
      ...
      {
        setBasicFilter: {
          filter: { range: { sheetId, startRowIndex: 0, startColumnIndex: 0, endColumnIndex: headerCols } }
        }
      }
    ];
